...
> 当前实现说明，更新于 2026-07-14。性能数字是阶段性实测，不是硬保证。
...
Body 模型是主要瓶颈。阶段测试平均约 16 FPS，5 秒窗口 P05/P50/P95 为 12/18/18，最低观察到 10～12 FPS。GStreamer caps 的 30 FPS 只是输入声明。
...
- `ILoveYou` 承担 trial0 的 `OK` 语义：优先确认待处理的收起请求，否则返回 FACE。
...
interval=3+KWS 测试中 Hand 平均 13.31 ms、P95 28.60 ms；它只表示单个 ROI 的 sidecar 推理耗时，不包含 Body、RGA、双手、编码和调度尾延迟。
