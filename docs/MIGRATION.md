...
> 初始迁移日期：2026-07-13；当前快照：2026-07-14，分支 `no-arm-test`。本文描述选择性移植，不表示整体合并来源项目。
...
`fourth` 的生产热路径已有 Body 腕点，因此直接裁腕部 ROI 后运行 landmark/classifier，不对整帧同步运行 palm detector。这样避免阻塞阶段平均约 16 FPS 的主视觉链。palm detector 模型保留，可用于离线或未来低频兜底研究。
...
- 映射：`Open_Palm/Closed_Fist/Pointing_Up` 控制情景/profile；`ILoveYou` 承担 OK 语义，优先确认收起，否则返回 FACE；
...
### 第一阶段：收口当前迁移（代码已提交，验收未完成）

- 保持 `no-arm-test` 提交可复现并修复生命周期残留；
- 双手+人脸+人体+推流长时间性能测试；
...
### 第三阶段：多模态输入（语音代码已接入，安全未开放）

- 为现有 KWS 增加阈值标定、唤醒/二次确认，禁止误触发开关机；
