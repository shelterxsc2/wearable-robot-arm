...
> **日期**: 2026-05-28  
> **适用版本**: `imu-main-test-roll+yaw`（Roll + Yaw 双轴控制）  
...
当前 UART 只有 10 字节裸 int16 发送。若要传回数据，建议复用已定义的帧协议：
...
```
[x, y, z, k1, k2]  // 10 字节 raw int16
```

> 为兼容现有协议，上位机发送不变。下位机新增反馈通道即可。
...
5. [ ] J4 的**初始基准角度**：当前代码 `NRF_SERVO1_DEG = 50.0f`，但文档写 90°。哪个是水平位？
