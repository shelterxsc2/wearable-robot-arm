...
> 文档状态：历史方案与标定背景。当前 `fourth` 已增加 FIRST_PERSON 和统一控制路由，实际 J4/J5 映射、限位和发令周期以 `src/rga_npu.cpp`、`src/first_person_control.cpp` 和 `CONTROL.md` 为准。本文方案未经重新核对的数字不能直接用于实机。
