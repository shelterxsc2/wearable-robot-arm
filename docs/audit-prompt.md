...
# 任务
读取以下文件，理解当前 `imu-3states-hat` 分支的实现状态，并给出一份简明扼要的项目状态汇报。
...
- 当前控制逻辑的状态（Pitch/Yaw 双轴、舵机控制设计、手势状态机）
- 已知问题和待办事项
...
11. **最新进度**：相比 `imu-3states-main`，`imu-3states-hat` 改动了什么关键逻辑？
    - A-inverse 矩阵解耦替代标量 baseline
    - 延迟 R_init 初始化（等待 move_complete）
    - Pitch 控制极性翻转
    - 控制轴从 Roll/wx 切换到 Pitch/wy
    - YUY2 30fps + RGA 硬件转码
    - bt_stub.c 替代 bluetooth_spp.c
    - uart_loopback_test.cpp 测试工具
12. **下一步**：J4 舵机标定准备状态 + Body 模型优化方向 + RuleEngine 手势扩展 + UART 协议升级

要求：简明扼要，不要大段粘贴代码，用工程师能理解的语言总结。重点突出 `imu-3states-hat` 相比前序分支的核心变化（A-inverse 解耦、延迟初始化、控制轴切换、极性翻转、YUY2 输入）。
