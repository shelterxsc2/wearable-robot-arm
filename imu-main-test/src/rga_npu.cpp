...
struct EndpointPredictor {
    ...
    void update(float wz, MotionState state) {
        ...
        float k;
        switch (state) {
            case STATE_ACCEL:            k = 0.50f; break;
            ...
            case STATE_STOP_TO_ACCEL:    k = 0.40f; break;
...
void nrf24_control_update(void)
{
    ...
    static bool nrf_baseline_set = false;
    ...
    static float last_cmd_target_yaw = 0.0f;  // 上一次发令的预测目标位置
    ...
    if (!nrf_baseline_set) {
        ...
        nrf_baseline_set = true;
        last_cmd_target_yaw = current_yaw_deg;
        predictor.reset();
    }

    /* 用 wz 历史（10ms 分辨率）更新运动上下文并运行状态机 */
    ctx.update_from_hist(wz_hist, wz_count, 5);   /* 取最近 5 个 ≈ 50ms */
    MotionState curr_state = next_motion_state(prev_state, ctx);
    prev_state = curr_state;

    /* 更新终点预测 */
    predictor.update(wz, curr_state);

    /* 只在状态变化时打印 */
    if (curr_state != prev_print_state) {
        printf("[NRF-STATE] wz=%+.2f pred=%+.1f° | %s\n",
               wz, predictor.pred_delta_yaw, motion_state_name(curr_state));
    ...
    /* 计算目标 yaw：
     * - 运动中：使用预测终点（current_yaw + pred_delta）
     * - 停止中：使用当前实际位置（消除预测残余误差）
     */
    float target_yaw_deg = is_stop ? current_yaw_deg
                                   : predictor.get_target_yaw(current_yaw_deg);

    /* 相对于上一次发令目标的变化 */
    float delta_yaw_deg = normalize_angle_deg(target_yaw_deg - last_cmd_target_yaw);
    bool moved_enough = std::fabs(delta_yaw_deg) > CMD_YAW_THRESHOLD_DEG;

    /* 自适应间隔：
     * - 主窗口（加速→匀速）：150ms，尽早确立预测
     * - 修正窗口（匀速→减速）：150ms，最后机会修正
     * - 常规修正：200ms
     * - 停止对准：200ms
     */
    ...
    if (should_cmd) {
        ...
        float target_x = NRF_FACE_X_CM + NRF_TRACK_DIST_CM * std::sin(cum_yaw_offset);
        float target_y = NRF_FACE_Y_CM + NRF_TRACK_DIST_CM * std::cos(cum_yaw_offset);
        float target_z = NRF_FACE_Z_CM;
        ...
        printf("[NRF-CMD] state=%s wz=%+.2f pred_delta=%+.1f° target_yaw=%+.1f° -> (%.1f, %.1f, %.1f)\n",
               motion_state_name(curr_state), wz, predictor.pred_delta_yaw,
               target_yaw_deg - nrf_baseline_yaw_deg,
